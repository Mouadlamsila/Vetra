...
export default function CallbackOfLogin() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // ✅ فك التوكن ديال Google باش تجيب الإيميل
        ...
        // ✅ تحقق واش المستخدم موجود فـ Strapi
        ...
        if (userExists) {
          ...
          // ✅ تخزين المعلومات ديال المستخدم فـ localStorage
          localStorage.setItem("token", accessToken); // ممكن تعوضو بـ JWT ديال Strapi
          localStorage.setItem("user", JSON.stringify(user.id));
          localStorage.setItem("IDUser", user.id);
          localStorage.setItem("role", "user");
          localStorage.setItem("userEmail", userEmail);
          localStorage.setItem("userName", user.username || userEmail.split("@")[0]);
        ...
        console.error("Erreur dans callback login:", error);
        setStatus("error");
