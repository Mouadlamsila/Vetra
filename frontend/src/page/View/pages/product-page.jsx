...
export default function ProductPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const ratingResponse = await axios.get(
          `http://localhost:1337/api/products/${id}?populate[rating_products][populate]=user`,
        )
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:1337/api/carts?filters[user][id][$eq]=${userId}&populate[product][populate]=*`,
        )
