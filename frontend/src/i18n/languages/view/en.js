import home from './home';
import { pagesEn } from './pages';
import { componentsEn } from './components';

export default {
  ...pagesEn,
  ...componentsEn,
  home: home.en,
  productDetails: {
    addToCart: "Add to Cart",
    addingToCart: "Adding product to cart...",
    buyNow: "Buy Now",
    outOfStock: "Out of Stock",
    inStock: "In Stock",
    quantity: "Quantity",
    description: "Description",
    specifications: "Specifications",
    reviews: "Reviews",
    relatedProducts: "Related Products",
    share: "Share",
    save: "Save",
    delivery: "Delivery",
    returns: "Returns",
    warranty: "Warranty",
    seller: "Seller",
    brand: "Brand",
    category: "Category",
    tags: "Tags",
    sku: "SKU",
    availability: "Availability",
    shipping: "Shipping",
    payment: "Payment",
    support: "Support",
    weight: "Weight",
    units: "Units",
    stock: "Stock",
    features: "Features",
    technicalSpecs: "Technical Specifications",
    home: "Home",
    addedToFavorites: "Product added to favorites successfully!",
    removedFromFavorites: "Product removed from favorites successfully!",
    onlyLeft: "Only {{count}} left",
    processing: "Processing...",
    addToFavorites: "Add to Favorites",
    shareSuccess: "Link copy successfully!",
    shareError: "Error sharing product",
    loading: "Loading...",
    loginToFavorite: "Please login to add product to favorites",
    loginRequired: "Please login to add to cart",
    favoriteError: "Error adding product to favorites",
    favoriteSuccess: "Product added to favorites successfully!",
    favoriteRemoveSuccess: "Product removed from favorites successfully!",
    favoriteRemoveError: "Error removing product from favorites",
    loginToReview: "Please login to write a review",
    error: {
      loginRequired: "Please login to add to cart",
      stockExceeded: "Sorry, only {{count}} units left in stock",
      cartUpdateError: "Error updating cart",
      cartAddError: "Error adding to cart",
      quantityUpdated: "Quantity updated in cart!",
      productAdded: "Product added to cart successfully!"
    },
    rateProduct: "Rate Product",
    writeReview: "Write a Review",
    ratingCount: "Rating Count",
    viewAllReviews: "View All Reviews",
    updateReview: "Update Review",
    rating: "Rating",
    review: "Review",
    submit: "Submit",
    cancel: "Cancel",
    reviewPlaceholder: "...write your review here",
    reviewSuccess: "Review submitted successfully!",
    reviewError: "Error submitting review",
    reviewUpdateSuccess: "Review updated successfully!",
    reviewUpdateError: "Error updating review",
    id: "ID",
    dimensions: "Dimensions",
    selectRating: "Please select a rating",
    productNotFound: "Product not found",
    lowStock: "Low Stock",
  },
  category: {
    filters: "Filters",
    sortBy: "Sort By",
    price: "Price",
    brand: "Brand",
    rating: "Rating",
    availability: "Availability",
    apply: "Apply",
    clear: "Clear",
    results: "Results",
    noResults: "No products found",
    loading: "Loading...",
    error: "Error loading products",
    grid: "Grid",
    list: "List",
    perPage: "per page",
    showing: "Showing",
    of: "of",
    products: "products",
    min: "Min",
    max: "Max",
    inStockOnly: "In Stock Only",
    andUp: "and up",
    resetAll: "Reset All",
    home: "Home",
    resetFilters: "Reset Filters",
    uncategorized: "Uncategorized",
    reset: "Reset Filters"
  },
  search: {
    searchResults: "Search Results",
    noResults: "No results found",
    suggestions: "Suggestions",
    popular: "Popular Searches",
    recent: "Recent Searches",
    clear: "Clear History",
    filters: "Search Filters",
    sortBy: "Sort By",
    relevance: "Relevance",
    price: "Price",
    newest: "Newest",
    rating: "Rating",
    loading: "Searching...",
    error: "Error performing search",
    tryAgain: "Try Again",
    refine: "Refine Search",
    categories: "Categories",
    brands: "Brands",
    priceRange: "Price Range"
  },
  store: {
    about: "About Store",
    contact: "Contact",
    location: "Location",
    hours: "Business Hours",
    products: "Products",
    categories: "Categories",
    reviews: "Store Reviews",
    rating: "Store Rating",
    followers: "Followers",
    following: "Following",
    joined: "Joined",
    verified: "Verified Seller",
    shipping: "Shipping Policy",
    returns: "Return Policy",
    payment: "Payment Methods",
    support: "Customer Support",
    social: "Social Media",
    website: "Website",
    email: "Email",
    phone: "Phone",
    address: "Address",
    description: "Store Description",
    policies: "Store Policies",
    terms: "Terms & Conditions",
    privacy: "Privacy Policy"
  }
}; 